...
//generated by image_conversion.mk
...
void redrawTile(u8* mem_start, u8 x, u8 y, u8 width, u8 height) {
	...
	//find out # of tiles that make height and width. Depends on the sprite size and if it's aligned
	//if not aligned with a tile we need to also pick the next one

	//move width to pixels and add the difference with alignment
	//Mode 0 so width*2 is pixels
	//divide by 8 (tile size) and round up
	//https://youtu.be/ONs987A4HOc?list=PL09EOWt_YmtVqW7GVpXIj7psRpzqNHPvx&t=200
	px_to_clean = (2*width) + (x % 4);
	new_width = (px_to_clean / 8) + ((px_to_clean % 8) !=0); //ROUNDUP - add one tile if MOD!=0
	//height - px and bytes are the same
	px_to_clean = height + (y % 8);
	new_height = (px_to_clean / 8) + ((px_to_clean % 8 ) !=0); //ROUNDUP 

	//first tile of tilemap to render (20 is number of tiles per row)
	first_tile = ((new_y - GAME_AREA_TOP) / 8) * 20 + (new_x / 4); 		//from "coords" to tiles

	cpct_etm_setDrawTilemap4x8_ag( new_width, new_height, 20, &g_tileset_00[0] );
	cpct_etm_drawTilemap4x8_ag( (u8*)cpct_getScreenPtr( mem_start, new_x, new_y), &map[first_tile] );
}

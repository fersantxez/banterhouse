...
def page_danger(c: canvas.Canvas) -> None:
    ...
    panel(c, 27, 38, W - 54, 70, INK, MAGENTA)
    c.setFillColor(white)
    c.setFont("Sans-Black", 9)
    c.drawString(41, 84, "BANTER")
    c.setFillColor(MAGENTA)
    c.drawString(41, 73, "HOUSE")
    c.setFillColor(ACID)
    c.setFont("Mono-Bold", 8.5)
    c.drawString(102, 79, "*07/12")
    c.setFillColor(GRAY)
    c.drawString(178, 79, "|||--")
    c.setFillColor(CYAN)
    c.drawString(239, 79, "CAFE 2")
    c.setFillColor(white)
    c.drawString(307, 79, "08450")
    c.setFont("Sans-Bold", 6.8)
    c.drawString(41, 55, "Logo · idea · Carga · cafés · puntuación. Sin cafés: GAME OVER.")
